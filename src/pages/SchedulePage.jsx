        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="group flex items-center gap-2 px-4 py-2 bg-[#3498DB] text-white font-semibold text-sm rounded-xl shadow-md shadow-blue-50 hover:bg-[#2980B9] transition-all"
            >
              <FaPlus className="text-xs" /> Add Schedule
            </button>
            <button className="p-2.5 bg-white border border-gray-100 text-gray-400 hover:text-[#3498DB] rounded-xl shadow-sm transition-all">
        ...
        <div className="flex items-center justify-between mb-6 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3">
            <button 
              ...
              className="w-8 h-8 flex items-center justify-center bg-gray-50 text-gray-400 hover:text-gray-800 rounded-lg transition-all"
            >
              <FaChevronLeft className="text-xs" />
            </button>
            <div className="text-center">
              <h2 className="text-base font-semibold text-gray-800">{formatDate(currentDate)}</h2>
          ...
          <button 
            ...
            className="px-4 py-1.5 bg-gray-50 text-gray-500 font-semibold text-[10px] uppercase tracking-widest rounded-lg hover:bg-gray-100 transition-all border border-gray-100"
          >
            Go to Today
          </button>
        </div>
        ...
        <div className="w-full">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1 group">
              <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 text-sm group-focus-within:text-[#3498DB] transition-colors" />
              <input 
                type="text" 
                placeholder="Find an activity..."
                className="w-full h-10 pl-10 pr-4 bg-white border border-gray-100 rounded-xl text-sm text-gray-700 focus:ring-2 focus:ring-blue-50 focus:border-blue-100 outline-none transition-all shadow-sm"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
            <div className="flex bg-white p-1 rounded-xl border border-gray-100 shadow-sm grow-0">
              {['All', 'Today', 'Upcoming', 'Completed'].map((filter) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg font-semibold text-xs transition-all ${
                    activeFilter === filter 
                      ? 'bg-[#3498DB] text-white shadow-md shadow-blue-50' 
          ...
          {filteredActivities.length > 0 ? (
            <div className="relative group">
              ...
              <button 
                onClick={() => scroll('left')}
                className="absolute left-0 top-1/2 -translate-y-1/2 -ml-6 z-20 w-14 h-14 bg-white rounded-full shadow-xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#3498DB] opacity-0 group-hover:opacity-100 transition-all hover:scale-110 active:scale-95"
              >
                <FaChevronLeft className="text-xl" />
              </button>
              <button 
                onClick={() => scroll('right')}
                className="absolute right-0 top-1/2 -translate-y-1/2 -mr-6 z-20 w-14 h-14 bg-white rounded-full shadow-xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#3498DB] opacity-0 group-hover:opacity-100 transition-all hover:scale-110 active:scale-95"
              >
              ...
              >
                {filteredActivities.map((activity) => (
                  <div 
                    key={activity.id}
                    className={`flex-shrink-0 w-full md:w-[calc(50%-1.5rem)] xl:w-[calc(25%-1.5rem)] snap-start group/card bg-white rounded-2xl p-4 transition-all duration-300 relative border cursor-grab active:cursor-grabbing hover:scale-[1.02] hover:shadow-xl hover:z-30 ${
                      activity.status === 'Ongoing'
                        ? 'border-[#3498DB] ring-4 ring-blue-50'
                  ...
                  >
                    ...
                    <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover/card:opacity-100 transition-all transform translate-y-2 group-hover/card:translate-y-0 z-10">
                      <button title="Edit" className="w-8 h-8 bg-white shadow-md border border-gray-50 text-gray-400 hover:text-blue-500 rounded-lg flex items-center justify-center transition-all text-xs"><FaEdit /></button>
                      <button title="Delete" className="w-8 h-8 bg-white shadow-md border border-gray-50 text-gray-400 hover:text-red-500 rounded-lg flex items-center justify-center transition-all text-xs"><FaTrashAlt /></button>
                      {activity.status !== 'Completed' && (
                        <button title="Mark Complete" className="w-8 h-8 bg-white shadow-md border border-gray-50 text-gray-400 hover:text-green-500 rounded-lg flex items-center justify-center transition-all text-xs"><FaCheck /></button>
                      )}
                    </div>
                    ...
                    <div className="mb-4 flex justify-between items-start">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg ${
                        ...
                        activity.status === 'Completed' ? 'bg-green-50 text-[#00bea3]' : 'bg-gray-50 text-gray-400'
                      }`}>
                        {activity.icon}
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider ${
                        activity.status === 'Ongoing' ? 'bg-blue-100 text-blue-600' :
                    ...
                    <div className="mb-4">
                      <h3 className="text-base font-semibold text-gray-900 mb-1 leading-tight">{activity.name}</h3>
                    ...
                    <div className="mb-6">
                      <div className="flex justify-between items-center mb-1.5">
                        <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Activity Progress</span>
                        <span className="text-[10px] font-semibold text-gray-700">{activity.progress}%</span>
                      </div>
                      <div className="w-full h-1.5 bg-gray-50 rounded-full overflow-hidden">
                        <div 
                          className={`h-full transition-all duration-1000 ${
                            activity.status === 'Ongoing' ? 'bg-[#3498DB]' : 
                            activity.status === 'Completed' ? 'bg-[#00bea3]' : 'bg-gray-200'
                          }`}
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-gray-50 mt-auto">
                      <div className="flex items-center -space-x-2">
                         {[1,2,3].map(i => (
                           <div key={i} className="w-6 h-6 rounded-full border border-white bg-gray-100 flex items-center justify-center text-[6px] font-semibold text-gray-400 overflow-hidden">
                             <img src={`https://i.pravatar.cc/100?u=${activity.id}-${i}`} alt="kid" className="w-full h-full object-cover" />
                           </div>
                         ))}
                         <div className="w-6 h-6 rounded-full border border-white bg-blue-50 flex items-center justify-center text-[7px] font-semibold text-[#3498DB] relative z-10">
                           +{activity.children - 3}
                         </div>
                      </div>
                      <div className="flex items-center gap-1.5 px-2 py-1 bg-gray-50 rounded-lg">
                        <FaMapMarkerAlt className="text-gray-300 text-[10px]" />
                        <span className="text-[9px] font-semibold text-gray-500">{activity.location}</span>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[40px] border border-dashed border-gray-200">
              <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center mb-6">
                <FaCalendarDay className="text-4xl text-gray-200" />
              </div>
              <h3 className="text-2xl font-black text-gray-800 mb-2">No schedule for today</h3>
              <p className="text-gray-400 font-bold">Try adjusting your filters or search query</p>
              <button 
                onClick={() => {setActiveFilter('All'); setSearchQuery('');}}
                className="mt-8 px-8 py-3 bg-gray-50 text-gray-500 font-black text-xs uppercase tracking-widest rounded-full hover:bg-gray-100 transition-all"
              >
                Reset All Filters
              </button>
            </div>
